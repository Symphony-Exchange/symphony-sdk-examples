...
export const ViemExample = () => {
  const symphonyConfig: SymphonyConfig = SymphonySDK.getConfig();
  const chainId: number | undefined = symphonyConfig.chainId;
  const nativeAddress: Address | undefined = symphonyConfig.nativeAddress;

  const [tokenList, setTokenList] = useState<TokenList | undefined>(undefined);
  const [walletClientViem, setWalletClientViem] = useState<
  ...
  const [routeDetails, setRouteDetails] = useState<any>(undefined);
  ...
  const connectViemDirect = async () => {
    try {
      if (typeof window !== "undefined" && (window as any).ethereum) {
        ...
        (walletClient as any).account = { address, type: "json-rpc" };
  ...
  const handleApprove = async () => {
    if (!route || !walletClientViem) return;
    try {
      const txHash: TransactionReceipt = await route.giveApproval();
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button
              onClick={async () => {
                SymphonySDK.setConfig({ nativeAddress: "0x1" as Address });
