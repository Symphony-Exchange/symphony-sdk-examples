...
import { Symphony, getRoute, Route, symphonyAbi } from "symphony-sdk/ethers";
import type {
  Address,
  SymphonyConfig,
  SwapResult,
...
import type { TokenList, TokenMetadata } from "symphony-sdk/ethers";
import {
  BrowserProvider,
  type Signer,
...
export const EthersExample = () => {
  ...
  const handleSwap = async () => {
    if (!route || !signer) return;
    try {
      // swap now returns SwapResult with TransactionReceipt (already waited)
      ...
      setTransactionHash(swapResult.swapReceipt.hash);
  ...
  return (
    <div className="h-full w-full flex flex-col gap-5 items-center justify-center">
      <div className="w-full max-w-4xl">
        <h2 className="text-2xl font-bold mb-4">Ethers v6 Example</h2>
